...
async function updatePriceIfNeeded(newPrice: number, currency: string, id: string, page: Page, book: Book, verbose: boolean) {
...
export async function updatePricing(book: Book, params: ActionParams): Promise<ActionResult> {
  ...
  const primaryMarketplace = await page.$eval(id, x => (x as HTMLSelectElement).value);
