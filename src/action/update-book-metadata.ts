...
export async function updateBookMetadata(book: Book, params: ActionParams): Promise<ActionResult> {
  ...
  if (!book.wasEverPublished) {
    ...
    const authorFirstName = await page.$eval(id, x => (x as HTMLInputElement).value);
    ...
    const authorLastName = await page.$eval(id, x => (x as HTMLInputElement).value);
    ...
    const illustratorFirstName = await page.$eval(id, x => (x as HTMLInputElement).value);
  ...
  const oldDescription = cleanupHtmlForAmazonDescription(await page.$eval('#cke_1_contents > textarea', x => (x as HTMLTextAreaElement).value) || '');
  ...
  const oldKeyword0 = isNew ? '' : (await page.$eval('#data-print-book-keywords-0', x => (x as HTMLInputElement).value)) || '';
  const oldKeyword1 = isNew ? '' : (await page.$eval('#data-print-book-keywords-1', x => (x as HTMLInputElement).value)) || '';
  const oldKeyword2 = isNew ? '' : (await page.$eval('#data-print-book-keywords-2', x => (x as HTMLInputElement).value)) || '';
  const oldKeyword3 = isNew ? '' : (await page.$eval('#data-print-book-keywords-3', x => (x as HTMLInputElement).value)) || '';
  const oldKeyword4 = isNew ? '' : (await page.$eval('#data-print-book-keywords-4', x => (x as HTMLInputElement).value)) || '';
  const oldKeyword5 = isNew ? '' : (await page.$eval('#data-print-book-keywords-5', x => (x as HTMLInputElement).value)) || '';
  const oldKeyword6 = isNew ? '' : (await page.$eval('#data-print-book-keywords-6', x => (x as HTMLInputElement).value)) || '';
