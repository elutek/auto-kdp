...
export function resolveAllValues(data: Map<string, string>, unresolvedKeys: Set<string>, allData: Array<Map<string, string>>) {
...
function _resolveStep(data: Map<string, string>, resolvedKeys: Set<string>, allData: Array<Map<string, string>>): boolean {
    let changedKey = null;
...
function _resolveOneValue(key: string, value: string, data: Map<string, string>, allData: Array<Map<string, string>>, resolvedKeys: Set<string>): string {
...
function _extractNeededKeys(value: string): Array<string> {
    let keys = [];
...
//    ${x} == 2
//    ${name} != Anna
function _resolveComparison(value: string): boolean {
    value = value.trim();
    if (value == 'true') {
        return true;
    }
...
function _resolveEquality(value: string): boolean {
    let equality = true;
    let j = value.indexOf('==');
    if (j < 0) {
        equality = false;
...
function _resolveNumberOrStringComparison(value: string): boolean {
    let less = true;
    let j = value.indexOf('<');
    if (j < 0) {
        j = value.indexOf('>');
    ...
    let val1: string | number = value.slice(0, j).trim();
    let val2: string | number = value.slice(j + (equalityToo ? 2 : 1)).trim();
...
//   "0    == 1"
//   "blah == 10 && ${x} == 10 || ${y} == 20"
function _resolveCondition(value: string): boolean {
    let orResult = false;
...
function _tryResolveConditionalSelector(value: string, sep1: string, sep2: string): string {
...
function _resolveConditionalSelector(value: string): string {
...
function _resolveVarbookref(value: string, allData: Array<Map<string, string>>): string {
    if (!value.includes('!!')) {
...
function _getResolvedValue(value: string, allData: Array<Map<string, string>>): string {
...
function _isResolved(value: string): boolean {
    return !value.includes('${') && !value.startsWith('$var');
}

function _getBookField(allData: Array<Map<string, string>>, searchKeys: string, fieldToExtract: string): string {
...
function _dataMatchesKeys(data: Map<string, string>, searchKeys: string): boolean {
